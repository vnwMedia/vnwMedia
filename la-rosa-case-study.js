(()=>{
  ...
  const metrics=[
    ['93,837','Ad Impressions','Standout Exposure in Local Search','Google Business Profile Search Ads delivered nearly 94,000 ad impressions across two restaurant locations.','Old Bridge + Tinton Falls'],
    ['1,325','Website Clicks','Local Discovery Becomes Website Interest','Google Business Profiles drove 1,325 clicks to the website, giving diners a direct path to explore La Rosa.','534 Old Bridge + 791 Tinton Falls'],
    ['29,736','Profile Views','A Strong Organic Search Presence','Nearly 30,000 organic Google Business Profile views put La Rosa’s restaurant information in front of people searching locally.','16,663 Old Bridge + 13,073 Tinton Falls'],
    ['TOP 3','Local Rankings','Top-Three Visibility Across Both Locations','Old Bridge: #1 for all 10 tracked keywords','Tinton Falls: Top 3 for all 10 tracked keywords, including 7 at #1']
  ];
  target.innerHTML=`<div class="shell"><div class="lr-location-layout"><aside class="lr-location"><p class="section-tag">CONFIRMED SERVICE SCOPE</p><strong>11<span>locations</span></strong><h2>A local search strategy for every restaurant.</h2><p>SEO for 11 locations, Google Business Profile Management, Google Business Profile Search Ads, and Reputation Management.</p><a href="../contact.html" class="pill pill-blue">Discuss your locations ↗</a></aside><div><div class="lr-heading"><p class="section-tag">RESULTS / OLD BRIDGE + TINTON FALLS</p><h2>Standout visibility. Meaningful interest.</h2></div><div class="lr-grid">${metrics.map((m,i)=>`<article class="lr-metric lr-metric-${i}"><span class="lr-index">0${i+1}</span><strong>${m[0]}</strong><span class="lr-metric-unit">${m[1]}</span><h3>${m[2]}</h3><p>${m[3]}</p><p class="lr-metric-detail">${m[4]}</p></article>`).join('')}</div></div></div><p class="lr-footnote">Results shown cover Old Bridge and Tinton Falls only. Rankings refer to 10 tracked keywords per location; they do not represent all 11 locations in our SEO scope.</p></div>`;
})();
