...
function workPageOne(){
  ...
  return `<section class="work work-agency-featured section" id="page-content"><div class="shell"><div class="work-agency-head reveal"><div><p class="section-tag">Selected Work</p><h2>Different industries. The same focus on clarity, trust, and action.</h2></div><p>Each project begins with a different business problem. The work below shows how strategy, design, content, search context, and lead paths come together around the customer’s next decision.</p></div><div class="work-grid">${DATA.cases.map((x,i)=>`<article class="work-card work-agency-card reveal">${x[1]==="la-rosa-chicken-grill"?`<figure class="work-larosa-media"><img src="${img(x[4])}" alt="La Rosa Chicken and Grill food spread"><img class="work-larosa-logo" src="${img("assets/la-rosa-logo-supplied.png")}" alt="La Rosa" width="218" height="53"></figure>`:`<img src="${img(x[4])}" alt="${esc(x[0])}">`}<div><span>${esc(x[2])}</span><h3>${esc(x[0])}</h3><p>${esc(x[3])}</p><ul aria-label="Services provided for ${esc(x[0])}">${projectScopes[i].map(scope=>`<li><a class="work-service-pill" href="${path(`services/${scopePages[scope]}.html`)}">${scope}</a></li>`).join("")}</ul><a class="work-case-link" href="${path(`case-studies/${x[1]}.html`)}" aria-label="Read ${esc(x[0])} case study"><b>Read case study ↗</b></a></div></article>`).join("")}</div><div class="portfolio-list reveal">${moreWork.map((x,i)=>`<a href="${path("contact.html")}"><span>${String(i+4).padStart(2,"0")}</span><strong>${x}</strong><i>Discuss this work ↗</i></a>`).join("")}</div></div></section>
